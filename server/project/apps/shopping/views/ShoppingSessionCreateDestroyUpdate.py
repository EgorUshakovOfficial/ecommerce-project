...
class ShoppingSessionCreateDestroyUpdate(APIView):
    def __init__(self, *args, **kwargs):
        ...
        self.MISSING_TOTAL = create_error("Missing total", "The total field is required")
        self.MISSING_USER = create_error("Missing Field", "The user field is required")

    # Extracts total and user from the request object
    # Args:
    ...
    def put(self, request):
        ...
        total, user_id = self.get_shopping_data(request)

        # If total does not exist, raise a missing field error
        if total is None:
            return Response(self.MISSING_TOTAL, status=status.HTTP_400_BAD_REQUEST)

        # Intialize shopping session data
        shopping_session_data = {"total":total, "user":user_id}
    ...
    def post(self, request):
        ...
        # If total does not exist, set it to 0
        if total is None:
            total = 0
